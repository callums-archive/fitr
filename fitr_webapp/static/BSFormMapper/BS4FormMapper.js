...
formGen.prototype = {
  ...
  attachError(field, error) {
    switch (field.type) {
      ...
      case 'confirm-password':
      case 'select':
        $(`#${field.id}`).parent().find('div[class="valid-feedback"]').remove()
        $(`#${field.id}`).parent().find('div[class="invalid-feedback"]').remove()
        $(`#${field.id}`).parent().append(`<div class="invalid-feedback">${error}</div>`);
        $(`#${field.id}`).removeClass("is-valid").addClass("is-invalid");
        break;

      case 'date':
        $(`#${field.id}_error`).find('div[class="valid-feedback"]').remove();
        $(`#${field.id}_error`).find('div[class="invalid-feedback"]').remove();
        $(`#${field.id}_error`).append(`<div class="invalid-feedback">${error}</div>`);
        $(`#${field.id}_year`).removeClass("is-valid").addClass("is-invalid");
        $(`#${field.id}_month`).removeClass("is-valid").addClass("is-invalid");
        $(`#${field.id}_day`).removeClass("is-valid").addClass("is-invalid");
        break;
    }
  },
  ...
  validField(field) {
    switch (field.type) {
      ...
      case 'date':
        $(`#${field.id}_error`).find('div[class="valid-feedback"]').remove();
        $(`#${field.id}_error`).find('div[class="invalid-feedback"]').remove();
        $(`#${field.id}_year`).removeClass("is-invalid").addClass("is-valid");
        $(`#${field.id}_month`).removeClass("is-invalid").addClass("is-valid");
        $(`#${field.id}_day`).removeClass("is-invalid").addClass("is-valid");
        break;
    }
  },
  ...
  validationMethods: {
    ...
    notEmpty() {
      ...
      if (!this.value || this.value == undefined || this.value == null || this.value.length == 0 || this.value == "") {
    ...
    isUsername() {
      ...
      if (/^[a-z0-9_]{3,16}$/.test(this.value) == false) {
        return [0, "Username supports upto 16 letters and numbers and underscores, minimum 3."];
      }
      return [1, ""];
    },
    isInteger() {
